...
### 3.0.5 发布记录（2026-09-08）

- 修复应用内安装把 OpenCode 版本探测失败误判为“未安装”的问题；检测到可执行文件但无法启动时改为展示修复状态。
- 一键安装与更新不再原地覆盖全局 npm 安装。新版本先安装到隔离临时目录并完成启动验证，再发布到用户目录下的插件专用版本目录；失败时保留现有版本和用户配置。
- Windows x64 使用兼容性更高的 baseline 包，安装超时会终止完整 npm 子进程树，避免 postinstall 长期占用文件。
- OpenCode 可执行文件损坏时只在首个空闲端口尝试一次启动，不再遍历 100 个端口重复失败并拖慢插件加载。
- 先安装验证、后重启插件管理的 OpenCode 服务；下载或验证失败不再中断当前可用服务。前端同步增加安装、更新、修复过程提示。
- 发布前执行 `pnpm.cmd exec tsc --noEmit`、`pnpm.cmd build`、IDEA 2023.2.4 SDK 下的 `gradle buildPlugin`、插件签名、敏感信息扫描和 Git 差异检查。
