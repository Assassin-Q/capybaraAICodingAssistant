...
export function McpSettings({ onChanged, projectPath }: McpSettingsProps) {
  ...
  return (
    <section className="flex min-h-0 flex-1 flex-col gap-4 overflow-hidden">
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 gap-4 overflow-y-auto lg:grid-cols-[15rem_minmax(0,1fr)] lg:overflow-hidden">
        <aside className="max-h-48 overflow-y-auto rounded-md bg-muted/30 p-2 lg:max-h-none"><p className="px-2 pb-2 pt-1 text-[11px] font-medium text-muted-foreground">已配置服务</p>{names.length === 0 ? <p className="px-2 py-4 text-xs text-muted-foreground">尚未配置 MCP</p> : <div className="flex flex-col gap-1">{names.map((name) => { const entry = config.mcp?.[name]; const status = statuses[name]; return <button className={cn("flex w-full items-center gap-2 rounded-md px-2 py-2 text-left hover:bg-accent", name === selectedName && "bg-secondary")} key={name} onClick={() => selectMcp(name)} type="button"><Server className="size-3.5 shrink-0 text-muted-foreground" /><span className="min-w-0 flex-1 truncate text-xs font-medium">{name}</span><span className={cn("size-1.5 rounded-full", entry?.enabled === false || status?.status === "disabled" ? "bg-muted-foreground" : status?.status === "connected" ? "bg-emerald-500" : "bg-amber-500")} /></button>; })}</div>}</aside>
        <div className="min-w-0 min-h-0 overflow-y-auto bg-muted/10 p-1 sm:p-4 lg:overscroll-contain"><div className="flex flex-wrap items-center justify-between gap-3 border-b border-border/50 px-2 pb-3 sm:px-0"><div><h3 className="text-base font-semibold">{selectedName ? "编辑 MCP 服务" : "新建 MCP 服务"}</h3><p className="mt-1 text-xs text-muted-foreground">表单适合常见配置，JSON 可维护完整高级字段。</p></div><div className="flex rounded-md bg-muted p-0.5"><Button onClick={() => { setMode("form"); setDraft(mode === "json" ? draftFromConfig(draft.name, normalizeJson(json, draft.name).config) : draft); }} size="xs" type="button" variant={mode === "form" ? "secondary" : "ghost"}>表单</Button><Button onClick={() => { setMode("json"); setJson(jsonFromDraft(draft)); }} size="xs" type="button" variant={mode === "json" ? "secondary" : "ghost"}>JSON</Button></div></div>
      ...
      {names.length > 0 && <div className="max-w-4xl overflow-hidden rounded-lg border border-border"><div className="border-b border-border px-4 py-2 text-xs font-medium text-muted-foreground">运行状态</div>{names.map((name) => { const entry = config.mcp?.[name]; const status = statuses[name]; return <div className="flex items-center gap-3 border-b border-border px-4 py-3 last:border-b-0" key={name}><span className="min-w-0 flex-1 truncate text-sm">{name}</span><span className="hidden text-xs text-muted-foreground sm:block">{entry?.type === "local" ? <><Terminal className="mr-1 inline size-3" />本地</> : <><Link2 className="mr-1 inline size-3" />远程</>}</span><Badge variant="secondary">{status ? statusLabel[status.status] : entry?.enabled === false ? "已停用" : "未连接"}</Badge><Switch aria-label={`启用 ${name}`} checked={entry?.enabled !== false} disabled={saving} onCheckedChange={(enabled) => void toggleMcp(name, enabled)} /></div>; })}</div>}
    </section>
  );
}
