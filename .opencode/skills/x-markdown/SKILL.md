...
| 当你需要... | 优先阅读 | 典型结果 |
| --- | --- | --- |
| 用最小配置渲染 Markdown | [CORE.md](references/CORE.md) | 用 `XMarkdown` 渲染基础内容 |
| 渲染 LLM 流式输出 | [STREAMING.md](references/STREAMING.md) | 正确处理 `hasNextChunk`、占位符、尾部指示器 |
| 把标签映射到业务组件 | [EXTENSIONS.md](references/EXTENSIONS.md) | 为自定义标签、代码块、富内容组件建立稳定映射 |
| 增加插件或主题定制 | [EXTENSIONS.md](references/EXTENSIONS.md) | 完成插件导入、主题类名接入、最小 CSS 覆盖 |
| 查看属性细节和默认值 | [API.md](references/API.md) | 获取 `XMarkdown` 与 streaming 的完整属性表 |

# 🛠 推荐工作流

1. 先看 [CORE.md](references/CORE.md)，先让基础渲染跑通。
2. 只有在内容是分块到达时，才继续看 [STREAMING.md](references/STREAMING.md)。
3. 只有在需要自定义标签、插件、主题时，才继续看 [EXTENSIONS.md](references/EXTENSIONS.md)。
4. 属性名与默认值以 [API.md](references/API.md) 为准，不要凭记忆猜。
...
- [CORE.md](references/CORE.md) - 包职责、安装与起步、安全默认值、常见渲染模式
- [STREAMING.md](references/STREAMING.md) - 分块渲染、不完整语法恢复、loading/done 行为
- [EXTENSIONS.md](references/EXTENSIONS.md) - 组件映射、插件、主题、自定义标签建议
- [API.md](references/API.md) - 从官方 `x-markdown` 文档生成的 API 参考
